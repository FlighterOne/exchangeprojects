...
def api_create_job_application():
    ...
    try:
        conn = sqlite3.connect(DB_PATH)
        cur = conn.cursor()
        _ensure_hr_schema(cur)
        cur.execute("SELECT hire_blocked, hire_block_reason FROM users WHERE user_id=?", (safe_int(user["id"]),))
        row = cur.fetchone()
        conn.close()
        if row and safe_int(row[0]):
            msg = row[1] or "Вы были уволены и не можете больше работать"
            return jsonify({"error": msg, "code": "hire_blocked"}), 403
    except Exception as e:
        logger.error("hire block check: %s", e)
...
@app.route("/api/admin/resign-requests", methods=["GET"])
def api_admin_resign_list():
    init_data = extract_init_data()
    if not init_data:
        return jsonify({"error": "No initData"}), 401
    user = validate_init_data(init_data)
    ...
    for row in cur.fetchall():
        d = dict(row)
        un = d.get("username")
        fn = ((d.get("first_name") or "") + " " + (d.get("last_name") or "")).strip()
        d["display_name"] = ("@" + un) if un else (fn or str(d["user_id"]))
        d["role_name"] = ROLES.get(safe_int(d.get("role")), "—")
        items.append(d)
    conn.close()
    return jsonify({"items": items})


@app.route("/api/admin/resign-requests/<int:rid>/status", methods=["POST"])
def api_admin_resign_status(rid):
    init_data = extract_init_data()
    if not init_data:
        return jsonify({"error": "No initData"}), 401
    user = validate_init_data(init_data)
    ...
    if new_status not in ("accepted", "rejected"):
        return jsonify({"error": "status accepted|rejected"}), 400
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    cur = conn.cursor()
    _ensure_hr_schema(cur)
    cur.execute("SELECT * FROM resign_requests WHERE id=?", (rid,))
    row = cur.fetchone()
    if not row:
        conn.close()
        return jsonify({"error": "Заявка не найдена"}), 404
    req = dict(row)
    if safe_int(req.get("user_id")) == safe_int(user["id"]) and get_user_role(user["id"]) < 6:
        conn.close()
        return jsonify({"error": "Вы не можете взаимодействовать с вашими обращениями"}), 403
    if req.get("status") != "pending":
        conn.close()
        return jsonify({"error": "Заявка уже обработана"}), 400
    now = datetime.now(MSK_TZ).isoformat()
    cur.execute("UPDATE resign_requests SET status=?, updated_at=? WHERE id=?", (new_status, now, rid))
    target = safe_int(req["user_id"])
    reason = req.get("reason") or "Заявка на увольнение"
    conn.commit()
    conn.close()
    if new_status == "accepted":
        fire_employee(target, "Увольнение по собственному желанию: " + reason, by_system=False)
        # если были активные санкции — hire_block уже, доп. причина
        notify_user(target, "Ваша заявка на увольнение <b>одобрена</b>.\nПричина: {}".format(reason.replace("<","")))
    else:
        notify_user(target, "Ваша заявка на увольнение <b>отклонена</b>.")
...
def api_admin_job_application_status(app_id):
    ...
    if safe_int(app.get("user_id")) == safe_int(user["id"]) and get_user_role(user["id"]) < 6:
        conn.close()
        return jsonify({"error": "Вы не можете взаимодействовать с вашими обращениями"}), 403
...
def notify_user(user_id, text, parse_mode="HTML"):
    try:
        bot.send_message(safe_int(user_id), text, parse_mode=parse_mode)
    except Exception as e:
        logger.error("notify_user %s: %s", user_id, e)


def min_shift_pay_for_role(role):
    """Оклад минимальной смены (6ч) по должности."""
    role = safe_int(role)
    if role >= 5:
        return 20.0
    return 10.0
...
def apply_staff_rating_delta(user_id, stars, comment=""):
    """Добавляет отзыв-звезду к рейтингу сотрудника. stars 1..5. Возвращает новый рейтинг."""
    ...
    try:
        cur.execute(
            ...
            "id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, stars INTEGER, comment TEXT, created_at TEXT)"
        )
        now = datetime.now(MSK_TZ).isoformat()
        cur.execute(
            "INSERT INTO staff_reviews (user_id, stars, comment, created_at) VALUES (?, ?, ?, ?)",
            (uid, max(1, min(5, safe_int(stars))), comment or "", now),
        )
        cur.execute("SELECT AVG(stars) FROM staff_reviews WHERE user_id=?", (uid,))
        avg = cur.fetchone()[0]
        if avg is None:
            avg = 5.0
        avg = float(avg)
        cur.execute("UPDATE users SET rating_staff=? WHERE user_id=?", (avg, uid))
        conn.commit()
        return avg
    except Exception as e:
        logger.error("staff rating: %s", e)
        return None
    finally:
        conn.close()


def fire_employee(user_id, reason, by_system=True):
    """Увольнение: роль 0, hire_blocked, уведомление с причиной."""
    uid = safe_int(user_id)
    if uid in MANDATORY_USERS:
        return False
    now = datetime.now(MSK_TZ).isoformat()
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    _ensure_hr_schema(cur)
    cur.execute(
        "UPDATE users SET role=0, hire_blocked=1, hire_block_reason=?, last_active=? WHERE user_id=?",
        (reason or "Увольнение", now, uid),
    )
    # отменить будущие смены
    try:
        cur.execute(
            "UPDATE shift_signups SET status='cancelled' WHERE user_id=? AND status='signed'",
            (uid,),
        )
    except Exception:
        pass
    ...
    msg = (
        "🚪 <b>Увольнение</b>\n\n"
        "Вы уволены с площадки X-Cod.\n"
        "<b>Причина:</b> {}\n\n"
        "Повторное трудоустройство недоступно."
    ).format((reason or "—").replace("<", ""))
    notify_user(uid, msg)
    return True


def apply_weekly_sanction(user_id, reason="Невыполнение недельного плана по сменам"):
    """Санкция: −25% к 3 ЗП, штраф = 25% мин. смены, 1★ в рейтинг сотрудника, автоувольнение при <3."""
    uid = safe_int(user_id)
    if uid in MANDATORY_USERS:
        return
    role = get_user_role(uid)
    if role < STAFF_MIN_ROLE or role >= 6:
        return
    now = datetime.now(MSK_TZ).isoformat()
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    _ensure_hr_schema(cur)
    # уже есть активная санкция за эту неделю — не дублируем (простая проверка по reason+сегодня)
    cur.execute(
        "SELECT id FROM user_sanctions WHERE user_id=? AND status='active' AND remaining_pays>0 LIMIT 1",
        (uid,),
    )
    if not cur.fetchone():
        cur.execute(
            "INSERT INTO user_sanctions (user_id, reason, remaining_pays, status, created_at) VALUES (?, ?, 3, 'active', ?)",
            (uid, reason, now),
        )
    # штраф 25% от мин. смены
    fine = round(min_shift_pay_for_role(role) * 0.25, 2)
    if fine > 0:
        cur.execute("UPDATE users SET balance = COALESCE(balance,0) - ? WHERE user_id=?", (fine, uid))
        cur.execute(
            "INSERT INTO transactions (user_id, bot_id, type, amount, description, created_at) VALUES (?, NULL, ?, ?, ?, ?)",
            (uid, "expense", -fine, "Штраф: санкция (−25% от мин. смены)", now),
        )
    conn.commit()
    conn.close()
    # 1 звезда
    rating = apply_staff_rating_delta(uid, 1, reason)
    notify_user(
        uid,
        "⚠️ <b>Санкции</b>\n\n"
        "Назначены санкции: −25% к следующим 3 зарплатам.\n"
        "Списан штраф: <b>{}$</b>\n"
        "Причина: {}\n"
        "Рейтинг сотрудника обновлён.".format(fine, reason.replace("<", "")),
    )
    if rating is not None and rating < 3.0:
        fire_employee(uid, "Рейтинг сотрудника ниже 3.0 (автоматическое увольнение)", by_system=True)


def check_and_apply_weekly_sanctions_for_user(user_id):
    """Если неделя закончилась и смен < 3 — санкция. Вызывается при активности."""
    uid = safe_int(user_id)
    role = get_user_role(uid)
    if role < STAFF_MIN_ROLE or role >= 6:
        return
    now = _msk_now()
    # проверяем прошлую полную неделю (пн-вс до текущего понедельника)
    from datetime import timedelta
    this_monday = now.date() - timedelta(days=now.weekday())
    prev_monday = this_monday - timedelta(days=7)
    prev_sunday = this_monday - timedelta(days=1)
    # только по понедельникам в первые сутки после недели, или если ещё не проверяли
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    _ensure_hr_schema(cur)
    cur.execute(
        "CREATE TABLE IF NOT EXISTS weekly_sanction_checks ("
        "user_id INTEGER, week_start TEXT, PRIMARY KEY(user_id, week_start))"
    )
    week_key = prev_monday.isoformat()
    cur.execute("SELECT 1 FROM weekly_sanction_checks WHERE user_id=? AND week_start=?", (uid, week_key))
    if cur.fetchone():
        conn.close()
        return
    # только если уже наступил новый понедельник
    if now.date() < this_monday:
        conn.close()
        return
    cur.execute(
        "SELECT COUNT(*) FROM shift_attendance WHERE user_id=? AND shift_date >= ? AND shift_date <= ? "
        "AND COALESCE(credit,'pending') NOT IN ('pending','none')",
        (uid, prev_monday.isoformat(), prev_sunday.isoformat()),
    )
    done = safe_int(cur.fetchone()[0])
    cur.execute(
        "INSERT OR IGNORE INTO weekly_sanction_checks (user_id, week_start) VALUES (?, ?)",
        (uid, week_key),
    )
    conn.commit()
    conn.close()
    if done < 3:
        apply_weekly_sanction(uid, "Невыполнение недельного плана по сменам ({}/3)".format(done))


def _ensure_hr_schema(cur=None):
    close = False
    if cur is None:
        conn = sqlite3.connect(DB_PATH)
        cur = conn.cursor()
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS user_sanctions (
            ...
            reason TEXT,
            remaining_pays INTEGER DEFAULT 3,
            status TEXT DEFAULT 'active',
            created_at TEXT
        )
    """)
    ...
    for col, ddl in [
        ("hire_blocked", "ALTER TABLE users ADD COLUMN hire_blocked INTEGER DEFAULT 0"),
        ("hire_block_reason", "ALTER TABLE users ADD COLUMN hire_block_reason TEXT"),
    ]:
        if col not in cols:
            try:
                cur.execute(ddl)
...
def _ensure_support_schema(cur=None):
    close = False
    if cur is None:
        conn = sqlite3.connect(DB_PATH)
        cur = conn.cursor()
...
def _get_or_create_support_thread(user_id):
    ...
    uid = safe_int(user_id)
    cur.execute("SELECT * FROM support_threads WHERE user_id = ?", (uid,))
    row = cur.fetchone()
    now = datetime.now(MSK_TZ).isoformat()
    if not row:
        cur.execute(
            "INSERT INTO support_threads (user_id, last_message_at, last_message_preview, user_unread, staff_unread, status, created_at) "
            "VALUES (?, ?, '', 0, 0, 'open', ?)",
            (uid, now, now),
        )
        conn.commit()
...
@app.route("/api/staff/resign", methods=["POST"])
def api_staff_resign():
    init_data = extract_init_data()
    if not init_data:
        return jsonify({"error": "No initData"}), 401
    user = validate_init_data(init_data)
    ...
    role = get_user_role(uid)
    if role < STAFF_MIN_ROLE or role >= 6:
        return jsonify({"error": "Недоступно для вашей роли"}), 403
    data = request.get_json(silent=True) or {}
    reason = (data.get("reason") or "").strip()
    if not reason:
        return jsonify({"error": "Укажите причину"}), 400
    if not data.get("confirm"):
        return jsonify({"error": "Подтвердите увольнение"}), 400
    now = datetime.now(MSK_TZ).isoformat()
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    _ensure_hr_schema(cur)
    cur.execute(
        "INSERT INTO resign_requests (user_id, reason, status, created_at, updated_at) VALUES (?, ?, 'pending', ?, ?)",
        (uid, reason, now, now),
    )
    rid = cur.lastrowid
    conn.commit()
...
def api_career():
    ...
    uid = safe_int(user["id"])
    try:
        check_and_apply_weekly_sanctions_for_user(uid)
    except Exception as e:
        logger.error("weekly sanction check: %s", e)
    role = get_user_role(uid)
    db = get_user(uid) or {}
    # stages: 3 intern, 4 mod, 5 admin
    stage = 0
    if role >= 5:
        stage = 2
    elif role >= 4:
    ...
    cur.execute(
        "SELECT COUNT(*) FROM shift_attendance WHERE user_id=? AND COALESCE(credit,'pending') NOT IN ('pending','none')",
        (uid,),
    )
    shifts_done = safe_int(cur.fetchone()[0])
    # days in system
    created = db.get("created_at") or ""
    days_in = 0
    try:
        if created:
            ...
            if c.tzinfo is None:
                c = c.replace(tzinfo=MSK_TZ)
            days_in = max(0, (_msk_now() - c).days)
    except Exception:
        days_in = 0
    # weekly shifts (Mon-Sun MSK)
    now = _msk_now()
    monday = (now.date() - __import__("datetime").timedelta(days=now.weekday())).isoformat()
    cur.execute(
        "SELECT COUNT(*) FROM shift_attendance WHERE user_id=? AND shift_date >= ? AND COALESCE(credit,'pending') NOT IN ('pending','none')",
        (uid, monday),
    )
    week_done = safe_int(cur.fetchone()[0])
    week_req = 3
    # progress to next stage
    # intern->mod: 12 shifts / 30 days
    # mod->admin: 150 shifts / 360 days + rating >= 4.75
    rating = float(db.get("rating_staff") or 5.0)
    if stage == 0:
        need_shifts, need_days = 12, 30
        prog_s = min(1.0, shifts_done / need_shifts) if need_shifts else 1
        prog_d = min(1.0, days_in / need_days) if need_days else 1
        progress = (prog_s + prog_d) / 2
        next_label = "Модератор"
    elif stage == 1:
        need_shifts, need_days = 150, 360
        prog_s = min(1.0, shifts_done / need_shifts)
        prog_d = min(1.0, days_in / need_days)
        prog_r = 1.0 if rating >= 4.75 else rating / 4.75
        progress = (prog_s + prog_d + prog_r) / 3
        next_label = "Админ"
    else:
        need_shifts, need_days = 0, 0
        progress = 1.0
        next_label = None
    _ensure_hr_schema(cur)
    cur.execute("SELECT remaining_pays, reason FROM user_sanctions WHERE user_id=? AND status='active' LIMIT 1", (uid,))
    san = cur.fetchone()
    conn.close()
    return jsonify({
        "role": role,
        "stage": stage,
        "stages": ["Стажер", "Модератор", "Админ"],
        ...
        "week_req": week_req,
        "rating_staff": rating,
        "sanction": {"remaining_pays": san[0], "reason": san[1]} if san else None,
    })


@app.route("/api/support/thread", methods=["GET"])
def api_support_thread():
    init_data = extract_init_data()
    if not init_data:
        return jsonify({"error": "No initData"}), 401
    user = validate_init_data(init_data)
    ...
    thread = _get_or_create_support_thread(user["id"])
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    cur = conn.cursor()
    _ensure_support_schema(cur)
    cur.execute(
        "SELECT id, sender_id, is_staff, body, created_at FROM support_messages WHERE thread_id = ? ORDER BY id ASC LIMIT 500",
        (thread["id"],),
    )
    messages = [dict(r) for r in cur.fetchall()]
    # mark user unread as read
    cur.execute("UPDATE support_threads SET user_unread = 0 WHERE id = ?", (thread["id"],))
    conn.commit()
    conn.close()
...
def api_support_send():
    ...
    _ensure_support_schema(cur)
    cur.execute(
        "INSERT INTO support_messages (thread_id, sender_id, is_staff, body, created_at) VALUES (?, ?, 0, ?, ?)",
        (thread["id"], user["id"], body, now),
    )
    preview = body[:120]
    cur.execute(
        "UPDATE support_threads SET last_message_at=?, last_message_preview=?, staff_unread=COALESCE(staff_unread,0)+1, status='open' WHERE id=?",
        (now, preview, thread["id"]),
    )
    conn.commit()
    mid = cur.lastrowid
    conn.close()
...
def api_admin_support_threads():
    ...
    if get_user_role(user["id"]) < MODERATOR_ROLE:
        return jsonify({"error": "Недостаточно прав"}), 403
    if get_user_role(user["id"]) < 6 and not user_on_duty_shift(user["id"]):
    ...
    cur.execute(
        ...
        WHERE COALESCE(t.last_message_preview, '') != '' OR t.staff_unread > 0
    ...
    for r in cur.fetchall():
        ...
        fname = ((d.get("first_name") or "") + " " + (d.get("last_name") or "")).strip()
        d["display_name"] = ("@" + uname) if uname else (fname or str(d["user_id"]))
        d["unread"] = safe_int(d.get("staff_unread") or 0)
        items.append(d)
    conn.close()
...
def api_admin_support_thread(thread_id):
    init_data = extract_init_data()
    if not init_data:
        return jsonify({"error": "No initData"}), 401
    user = validate_init_data(init_data)
    ...
    cur = conn.cursor()
    _ensure_support_schema(cur)
    cur.execute("SELECT t.*, u.username, u.first_name, u.last_name FROM support_threads t LEFT JOIN users u ON u.user_id=t.user_id WHERE t.id=?", (thread_id,))
    row = cur.fetchone()
    if not row:
        conn.close()
        return jsonify({"error": "Чат не найден"}), 404
    thread = dict(row)
    uname = thread.get("username")
    fname = ((thread.get("first_name") or "") + " " + (thread.get("last_name") or "")).strip()
    thread["display_name"] = ("@" + uname) if uname else (fname or str(thread["user_id"]))
    cur.execute(
        "SELECT id, sender_id, is_staff, body, created_at FROM support_messages WHERE thread_id=? ORDER BY id ASC LIMIT 500",
        (thread_id,),
    )
    messages = [dict(r) for r in cur.fetchall()]
    cur.execute("UPDATE support_threads SET staff_unread=0 WHERE id=?", (thread_id,))
    conn.commit()
    conn.close()
    return jsonify({"thread": thread, "messages": messages})


@app.route("/api/admin/support/threads/<int:thread_id>/messages", methods=["POST"])
def api_admin_support_reply(thread_id):
    init_data = extract_init_data()
    if not init_data:
        return jsonify({"error": "No initData"}), 401
    user = validate_init_data(init_data)
    ...
    cur = conn.cursor()
    _ensure_support_schema(cur)
    cur.execute("SELECT * FROM support_threads WHERE id=?", (thread_id,))
    row = cur.fetchone()
    if not row:
        conn.close()
        return jsonify({"error": "Чат не найден"}), 404
    thread = dict(row)
    cur.execute(
        "INSERT INTO support_messages (thread_id, sender_id, is_staff, body, created_at) VALUES (?, ?, 1, ?, ?)",
        (thread_id, user["id"], body, now),
    )
    preview = body[:120]
    cur.execute(
        "UPDATE support_threads SET last_message_at=?, last_message_preview=?, user_unread=COALESCE(user_unread,0)+1 WHERE id=?",
        (now, preview, thread_id),
    )
    conn.commit()
    conn.close()
    try:
        bot.send_message(thread["user_id"], f"💬 <b>Поддержка X-Cod</b>\n\n{body}", parse_mode="html")
    except Exception:
        pass
...
def api_restrict_publish():
    ...
    try:
        notify_user(target_id, (
            "⚠️ <b>Ограничение публикации</b>\n\n"
            "Модерация ограничила публикацию объявлений.\n"
            "<b>Срок:</b> {} дн.\n"
            "<b>Причина:</b> {}"
        ).format(days, (reason or "не указана").replace("<", "")))
    except Exception:
        pass
    ...
    try:
        ...
        try:
            ban_msg = (
                ...
                "Администрация сервиса заблокировала вам доступ.\n"
                "<b>Срок:</b> {} дн.\n"
                "<b>Причина:</b> {}"
            ).format(days, (reason or "не указана").replace("<", ""))
            notify_user(target_id, ban_msg)
        except Exception as e:
            logger.error("ban notify: %s", e)
...
def _finalize_shift_session(cur, sd, st, closer_id=None, now=None):
    """Закрыть смену и начислить зарплату по отметкам credit. Возвращает paid list."""
    now = now or _msk_now()
    ...
    cur.execute(
        "SELECT user_id, credit, COALESCE(credit_amount, 0), credited_at FROM shift_attendance "
    ...
    for row in cur.fetchall():
        ...
        already = row[3] if len(row) > 3 else None
        if already:
            continue
        if amt <= 0:
            continue
        # санкции: -25% если есть active sanction
        final_amt = amt
        sanction_note = ""
        try:
            cur.execute(
            ...
            san = cur.fetchone()
            if san:
                final_amt = round(amt * 0.75, 2)
                sanction_note = " (из-за санкций −25%)"
                cur.execute(
                    "UPDATE user_sanctions SET remaining_pays = remaining_pays - 1 WHERE id=?",
                    (san[0],),
                )
                cur.execute("SELECT remaining_pays FROM user_sanctions WHERE id=?", (san[0],))
                left = cur.fetchone()
                if left and safe_int(left[0]) <= 0:
                    cur.execute("UPDATE user_sanctions SET status='done' WHERE id=?", (san[0],))
        except Exception as e:
            logger.error("sanction apply: %s", e)
        try:
            cur.execute("UPDATE users SET balance = COALESCE(balance,0) + ? WHERE user_id = ?", (final_amt, suid))
            desc = "Зарплата за смену {0} {1}: {2}{3}".format(sd, st, credit, sanction_note)
            cur.execute(
                "INSERT INTO transactions (user_id, bot_id, type, amount, description, created_at) VALUES (?, NULL, ?, ?, ?, ?)",
                (suid, "salary", final_amt, desc, now_iso),
            )
            try:
                cur.execute(
                    "UPDATE shift_attendance SET credited_at=?, credit_amount=? WHERE shift_date=? AND shift_type=? AND user_id=?",
                    (now_iso, final_amt, sd, st, suid),
                )
            except Exception:
                cur.execute(
            ...
            paid.append({"user_id": suid, "amount": final_amt, "credit": credit, "gross": amt, "sanction": bool(sanction_note)})
        except Exception as e:
            logger.error("shift close pay %s: %s", suid, e)
    return paid
...
def _find_control_shift(cur, now=None):
    """
    Активная смена для контроля:
    - пока сессия open — показываем её (даже после 21/09, до автозакрытия 22/10);
    - новую по времени не показываем, пока предыдущая не закрыта;
    - после автозакрытия — календарная текущая.
    """
    from datetime import timedelta
    now = now or _msk_now()
    try:
        ...
        logger.error("find_control auto_close: %s", e)

    cur.execute(
        "SELECT shift_date, shift_type FROM shift_sessions WHERE status='open' "
        "ORDER BY COALESCE(opened_at, '') DESC LIMIT 1"
    )
    row = cur.fetchone()
    if row:
        return (row[0], row[1])

    # сессий open нет — ищем смену в окне [start, deadline), не закрытую
    today = now.date()
    candidates = []
    for delta in (-1, 0, 1):
        d = (today + timedelta(days=delta)).isoformat()
        for st in ("day", "night"):
            ...
            if start <= now < deadline:
                candidates.append((d, st, start, end, deadline))
    if not candidates:
        return None
    # берём самую раннюю по start среди подходящих, у которой нет status=closed
    candidates.sort(key=lambda x: x[2])
    for d, st, start, end, deadline in candidates:
        cur.execute(
            "SELECT status FROM shift_sessions WHERE shift_date=? AND shift_type=?",
            (d, st),
        )
        sess = cur.fetchone()
        ...
        if status == "closed":
            continue
        # не закрыта (нет сессии или open уже обработан выше) — активна
        return (d, st)
    # все в окне закрыты — если есть «чистая» текущая по календарю
    cur_shift = _find_current_shift(now)
    if not cur_shift:
        return None
    return (cur_shift[0], cur_shift[1])
...
def api_shifts_control():
    ...
    for r in cur.fetchall():
        ...
        deadline = end + _td(hours=1)
        cur.execute("SELECT status FROM shift_sessions WHERE shift_date=? AND shift_type=?", (sd, st))
        sess = cur.fetchone()
        status = sess[0] if sess else None
        # показывать незакрытую смену до автозакрытия
        if status == "closed":
            continue
        if now < deadline or status == "open":
            my_shifts.append({
                "shift_date": sd,
                "shift_type": st,
                "start": start.isoformat(),
                "end": end.isoformat(),
                "in_progress": start <= now < end or status == "open",
                "session_status": status,
            })

    current = _find_control_shift(cur, now)
    role = get_user_role(uid)
...
def api_shifts_control_attendance():
    ...
    try:
        start, end = _shift_bounds(sd, st)
        if now < start:
            return jsonify({"error": "Смена ещё не началась — отмечать нельзя"}), 400
        if now >= end:
            return jsonify({"error": "Смена уже закончилась — отмечать нельзя"}), 400
    except Exception as e:
        logger.error("attendance bounds: %s", e)
        return jsonify({"error": "Ошибка времени смены"}), 400
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    _ensure_shifts_schema(cur)
    cur.execute("SELECT 1 FROM shift_signups WHERE user_id=? AND shift_date=? AND shift_type=? AND status='signed'", (uid, sd, st))
...
def api_shifts_control_credit():
    ...
    if credit not in ("full", "none", "partial_8", "partial_6"):
        return jsonify({"error": "Некорректный тип зачёта"}), 400
    # ставки: модер/стажёр 25/15/10; админ+ 50/30/20
    target_role = get_user_role(target)
    if target_role >= ROLE_CHANGE_ROLE:  # 5+
        amounts = {"full": 50.0, "none": 0.0, "partial_8": 30.0, "partial_6": 20.0}
    else:
...
@safe_execute
def update_balance(user_id, amount, conn=None, cursor=None):
    user_id = safe_int(user_id)
    try:
...
def add_transaction(user_id, transaction_type, amount, description="", bot_id=None):
    ...
    cursor.execute('''
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    ''', (safe_int(user_id), bot_id, transaction_type, float(amount or 0), description, now))
    conn.commit()
    conn.close()
    return True
